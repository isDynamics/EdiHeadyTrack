...
camera = eht.Camera()
tracking_video = eht.Video(filename='resources/testvid.mp4')
...
head = eht.Head(facedetector=mediapipe).apply_filter(filter)
...
comparison_plot = eht.Plot(uhead, head, wax9).plot_property(property='acceleration', 
                                               xlim=(0.85, 1.35), 
                                               ylim=(-220, 220), 
                                               key_times=key_times) 